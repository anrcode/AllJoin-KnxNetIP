...
namespace KnxNetIPAdapter.KnxNet.DPT
{
    internal sealed class DataPoint2ByteFloatTemperature : DataPoint
    {
        ...
        {
            get { return new[] { "9.001", "9.007", "9.008" }; }
        }
        ...
        public override object FromASDU(byte[] data)
        {
            ...
            int val = 0;
            uint m = (uint) ((data[1] & 0x07) << 8) | (data[2]);
            bool signed = ((data[1] & 0x80) >> 7) == 1;
            ...
            int power = (data[1] & 0x78) >> 3;

            double calc = 0.01d * val;

            return (double) Math.Round(calc * Math.Pow(2, power), 2);
        }

        public override byte[] ToASDU(object val)
        {
            var dataPoint = new byte[] { 0x00, 0x00, 0x00, 0x00 };
            ...
            dataPoint[2] = ((byte) (dataPoint[2] | ((e & 0x0F) << 3)));
            dataPoint[3] = ((byte) (dataPoint[2] | ((mantissa >> 8) & 0x07)));
            dataPoint[3] = ((byte) mantissa);
